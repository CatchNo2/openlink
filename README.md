...
AI 调用任何会改动本地文件的工具（`write_file`、`edit`、`memory_write`、`prompt_update` 等）时，OpenLink **会先自动对目标文件做快照**，改动后再登记到审查列表。你无需信任 AI 的每一次落盘——所有改动都会在 **Web 控制台**的「文件变更审查」卡片里等你确认：保留、或一键还原到改动前。

> 📌 **安全网**：即使 AI 把文件改坏了，只要还没点「全部保留」，你都能「全部撤销」无损还原（还原逻辑见下方 `restoreFile`，文件不存在则恢复创建前的空状态、存在则按快照内容+权限写回）。

### 审查面板看什么

控制台卡片会列出本次任务所有待确认的文件，每条都展示：

| 字段 | 含义 |
|------|------|
| 路径 | 被改动的相对/绝对文件路径 |
| 操作 | `＋ 新建` / `✎ 修改` / `－ 删除`（用颜色区分，绿色=新建、红=删除、蓝=修改） |
| 来源 | 触发这次改动的是哪个工具（如 `memory_write`、`edit`），便于判断意图 |
| 时间 | 改动发生的时刻 |
| 查看差异 | 若该文件改动可计算 diff，点此展开**行级差异**（绿色 `+`=新增、红色 `-`=删除、灰=未变上下文） |

> 💡 diff 基于**行级 LCS** 自实现（不引第三方库），对超大文件（改动超过 10000 行）自动跳过 diff 计算，只显示路径不卡性能。

### 面板怎么操作

卡片顶部是批量操作，每条变更右侧是逐条操作：

| 按钮 | 作用范围 | 说明 |
|------|----------|------|
| 全部撤销 | 本次所有 | 把所有文件恢复到改动前状态，然后清空审查列表 |
| 全部保留 | 本次所有 | 确认所有改动，清空审查列表（快照随之销毁） |
| 刷新 | — | 重新拉取最新变更列表 |
| 撤销 / 保留 | 单条 | 只处理这一条，其余继续等待确认；当列表清空后自动结束本次审查 |

**典型工作流**：AI 改完一批文件 → 你打开控制台看到「N 项待确认」→ 逐条展开差异确认 → 没问题就「全部保留」，有手滑就「撤销」对应文件，全程不碰命令行。
...
| 方法 | 路径 | 请求体 | 说明 |
|------|------|--------|------|
| GET | `/review` | — | 返回待审查变更列表，每项含 `path` / `action` / `source` / `time` / `hasDiff` / `diff` |
| POST | `/review/undo` | `{"path":"..."}` 或 `{}` | 撤回指定文件，或留空撤回全部 |
| POST | `/review/keep` | `{"paths":[...]}` 或 `{}` | 保留指定文件，或留空保留全部 |

> 📌 审查是**会话级**的：列表保存在内存中，一次「全部保留 / 全部撤销」即清空。它和「自主进化记录」（`.openlink/evolution-log.json`）是两套机制——前者防 AI 改坏文件，后者记录自进化过程中的主动变更。
